...
// Calculate and print highest Average 

if ((sum_quebec / count_quebec) > max_avg) { // Check if Quebec has the highest average
    max_avg = (sum_quebec / count_quebec);
}
if ((sum_alberta / count_alberta) > max_avg) { //Check if Alberta has the highest average
    max_avg = (sum_alberta / count_alberta);
}
if ((sum_ontario / count_ontario) > max_avg) { //Check if Ontario has the highest average 
    max_avg = (sum_ontario / count_ontario);
}
if ((sum_british_columbia / count_british_columbia) > max_avg) { //Check if British Columbia has the highest average
    max_avg = (sum_british_columbia / count_british_columbia);
}
//Print the province with the highest average
...
if (lowest_avg == (sum_quebec / count_quebec)) {
 ...
 printf("The National Average of Diabetes is: %lf", national_average); 
 
 if ((sum_quebec / count_quebec) < national_average) {
	 printf("The average in Quebec is lower than the national average"); //check if averages for provinces are lower or higher than the national average 
 }
 if ((sum_quebec / count_quebec) > national_average) {
